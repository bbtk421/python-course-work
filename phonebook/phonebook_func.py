...
def addToList(self):
    ...
    var_fullname = "{} {}".format(
        var_fname, var_lname
    )  # combines out normalize dname into a full name
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(
                    self
                )  # call the fucntion to clear all of the text boxes and the selected index of listbox
                onRefresh(self)  # update the lisbox of the changes
                conn.commit()
        else:
            confirm = messagebox.showerror(
                "Last Record Error",
                "({}) is the last record in the database and cannot be deleted at this time".format(
                    var_select
                ),
            )
        conn.close()
    ...
    if (len(var_phone) > 0) and (
        ...
        with conn:
            ...
            if (
                ...
                messagebox.showinfo(
                    "No changes detected",
                    "Both ({}) and ({}) \nalready exist in the database for this name. \n\nYour update request has been canceled.".format(
                        var_phone, var_email
                    ),
                )
            onClear(self)
